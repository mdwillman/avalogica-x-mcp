...
export const getRecentPostsTool = {
    definition: {
        ...
        description: "Fetch the authenticated user's most recent X posts (e.g., last 20–100).",
        inputSchema: {
    ...
    handler: async (args) => {
        ...
        try {
            ...
            const posts = await client.getRecentPosts(creds.accessToken, creds.twitterUserId, limit);
            const payload = {
                posts,
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to fetch recent posts: ${msg}`);
        }
    },
};
