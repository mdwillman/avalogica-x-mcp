...
function normalizeFocus(focus) {
    if (!focus)
        return "all";
    if (focus === "tone" || focus === "topics" || focus === "interests")
        return focus;
    return "all";
}
export const summarizePostHistoryTool = {
    definition: {
        ...
        description: "Analyze a user's recent X posts to summarize tone, topics, and key interests.",
        inputSchema: {
            ...
            properties: {
                ...
                limit: {
                    type: "number",
                    default: 50,
                    description: "Number of recent posts to analyze (10–100). Default 50.",
                },
                focus: {
                    type: "string",
                    enum: ["all", "tone", "topics", "interests"],
                    description: "Optional focus area: 'tone', 'topics', 'interests', or 'all'. Default 'all'.",
                },
            },
            required: ["userId"],
        },
    ...
    handler: async (args) => {
        ...
        try {
            ...
            const posts = await xClient.getRecentPosts(creds.accessToken, creds.twitterUserId, limit);
            if (!posts.length) {
                throw new McpError(ErrorCode.InternalError, "No posts available to summarize.");
            }
            const corpus = posts
...
`;
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to summarize post history: ${msg}`);
        }
    },
};
