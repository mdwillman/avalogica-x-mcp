...
export const postToXTool = {
    definition: {
        name: "post_to_x",
        description: "Post a new update to X (Twitter) on behalf of the authenticated user. Assumes Dedalus Bridge has already authenticated the user and provides a trusted userId.",
        inputSchema: {
    ...
    handler: async (args) => {
        ...
        if (!blurb) {
            throw new McpError(ErrorCode.InvalidParams, "Missing or empty 'blurb'.");
        }
        try {
            ...
            console.log(`[avalogica-x-mcp] post_to_x – posted tweet for userId ${userId}: ${tweetUrl}`);
            const payload = { ok: true, tweetUrl };
            return {
                content: [
                    {
                        type: "text",
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to post to X: ${msg}`);
        }
    },
};
