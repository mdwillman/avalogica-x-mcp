# Contributing to Avalogica X MCP

Thank you for improving the Avalogica X MCP server! This project has two supported workflows for shipping new tools:
...
- Ensure fingerprints stay in responses (`[served by avalogica-x-mcp]`).
- Open a PR with a clear summary, test notes, and links to any newly added docs.

## Fingerprint Placement

All tools in Avalogica X MCP must include the fingerprint string:

`[served by avalogica-x-mcp]`

This fingerprint should appear in:
- Tool responses generated by handlers (e.g., appended to returned text blocks)
- The `/health` route response for the HTTP transport
- Any debug or diagnostic endpoints added by future contributors

The fingerprint helps Dedalus Bridge and internal routing validate that responses originate from this MCP server.
