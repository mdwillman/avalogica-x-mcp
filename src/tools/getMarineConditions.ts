...
export const getMarineConditionsTool = {
  definition: {
    ...
    description:
      "Get marine conditions (wave height, direction, period, and sea surface temperature) for the given coordinates over the next few hours.",
    inputSchema: {
      ...
      properties: {
        latitude: {
          type: "number",
          description:
            "Latitude in decimal degrees (e.g., 36.6000 for Monterey Bay)",
        },
        longitude: {
          type: "number",
          description:
            "Longitude in decimal degrees (e.g., -121.9000 for Monterey Bay)",
        },
        hours: {
          type: "number",
          default: 24,
          ...
          maximum: 120,
          description:
            "Number of forecast hours to summarize (default: 24, max: 120)",
        },
      },
   ...
   */
  handler: async (
    args: GetMarineConditionsArgs
  ): Promise<CallToolResult> => {
    try {
      const { latitude, longitude, hours = 24 } = args;

      const data: MarineConditionsResponse =
        await weatherClient.getMarineConditions(
          latitude,
          longitude,
          hours
        );
      ...
      if (!hourly.time || hourly.time.length === 0) {
        throw new Error(
          "No marine conditions data returned for the requested range"
        );
      }
      ...
      const maxWaveHeight =
        hourly.wave_height && hourly.wave_height.length > 0
      ...
      if (typeof waveDir === "number") {
        lines.push(
          `- Wave direction: ${waveDir.toFixed(
            0
      ...
      if (typeof sst === "number") {
        lines.push(
          `- Sea surface temperature: ${sst.toFixed(1)} °C`
        );
      }
      ...
      if (typeof maxWaveHeight === "number") {
        lines.push(
          "",
          `Max wave height over next ${hours}h: ${maxWaveHeight.toFixed(
            2
          )} m`
        );
      }
      ...
      lines.push(
        "",
        `Data source: Open-Meteo Marine API (hourly forecast)`
      );
      ...
      console.error(
        "[Weather MCP] Marine conditions handler error:",
        err
      );
      return {
        content: [
          {
            type: "text",
            text: `Error fetching marine conditions: ${
