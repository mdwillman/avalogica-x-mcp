...
export const getHourlyForecastTool = {
  definition: {
    ...
    description:
      "Get an hourly weather forecast for the given coordinates (default 24 hours). Returns hourly temperature and precipitation probability.",
    inputSchema: {
      ...
      properties: {
        latitude: {
          type: "number",
          description:
            "Latitude in decimal degrees (e.g., 34.05 for Los Angeles)",
        },
        longitude: {
          type: "number",
          description:
            "Longitude in decimal degrees (e.g., -118.25 for Los Angeles)",
        },
        hours: {
          type: "number",
          default: 24,
          ...
          maximum: 48,
          description:
            "Number of forecast hours to summarize (default: 24, max: 48)",
        },
      },
   ...
   */
  handler: async (
    args: GetHourlyForecastArgs
  ): Promise<CallToolResult> => {
    try {
      const { latitude, longitude, hours = 24 } = args;

      const data: HourlyForecastResponse =
        await weatherClient.getHourlyForecast(latitude, longitude, hours);
      ...
      const precip = (hourly.precipitation_probability ?? []).slice(
        0,
        sliceCount
      );
      ...
      const minTemp =
        validTemps.length > 0 ? Math.min(...validTemps) : undefined;
      const maxTemp =
        validTemps.length > 0 ? Math.max(...validTemps) : undefined;

      // Build a compact hourly table (e.g., every 3 hours to keep it readable)
      const lines: string[] = [];
      lines.push(
        `Hourly forecast for ${data.latitude.toFixed(
          3
        )}, ${data.longitude.toFixed(3)} (${timezone}), next ${sliceCount}h:`,
        ""
      );
      ...
      for (let i = 0; i < sliceCount; i += step) {
        ...
        const precipLabel =
          typeof p === "number" ? `${p.toFixed(0)}%` : "n/a";
      ...
      if (minTemp !== undefined && maxTemp !== undefined) {
        lines.push(
          "",
          `Temperature range over next ${sliceCount}h: ${minTemp.toFixed(
            1
          )}°C – ${maxTemp.toFixed(1)}°C`
        );
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error fetching hourly forecast: ${
