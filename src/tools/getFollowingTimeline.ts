// src/tools/getFollowingTimeline.ts
import type { CallToolResult } from "@modelcontextprotocol/sdk/types.js";
import { ErrorCode, McpError } from "@modelcontextprotocol/sdk/types.js";
import type {
  GetFollowingTimelineArgs,
  GetFollowingTimelineResult,
} from "../types.js";
...
export const getFollowingTimelineTool = {
  definition: {
    ...
    description:
      "Fetch the authenticated user's reverse-chronological home timeline on X (i.e., the 'Following' feed).",
    inputSchema: {
      ...
      properties: {
        ...
        limit: {
          ...
          default: 50,
          description:
            "Max number of posts to fetch from the Following timeline (5–100). Default 50.",
        },
        sinceId: {
          type: "string",
          description:
            "Only return posts more recent than this post ID (for incremental refresh).",
        },
        untilId: {
          type: "string",
          description:
            "Only return posts at or older than this post ID (for paging backwards).",
        },
      },
      required: ["userId"],
  ...
  handler: async (
    ...
    try {
      ...
      const { posts, nextToken, prevToken } =
        await client.getFollowingTimeline(
          creds.accessToken,
          creds.twitterUserId,
          {
            limit,
      ...
      const payload: GetFollowingTimelineResult = {
        posts,
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to fetch Following timeline: ${msg}`,
      );
    }
  },
};