...
function categorizeUsAqi(aqi: number): string {
...
function categorizeEuropeanAqi(aqi: number): string {
...
export const getAirQualityTool = {
  definition: {
    ...
    description:
      "Get an air quality summary (US & European AQI plus PM2.5/PM10) for the given coordinates over the next few hours.",
    inputSchema: {
   ...
   */
  handler: async (args: GetAirQualityArgs): Promise<CallToolResult> => {
    try {
      const { latitude, longitude, hours = 24 } = args;

      const data: AirQualityResponse = await weatherClient.getAirQuality(
        latitude,
        longitude,
        hours
      );
      ...
      const usCategory =
        typeof usAqi === "number" ? categorizeUsAqi(usAqi) : "unknown";
      const euCategory =
        typeof euAqi === "number" ? categorizeEuropeanAqi(euAqi) : "unknown";
      ...
      lines.push(
        "",
        `Summary window: next ${hours}h (Open-Meteo Air Quality API)`
      );
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error fetching air quality: ${
